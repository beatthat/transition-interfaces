...
namespace BeatThat.Transitions
{
    ...
    [EditDefine("TRANSITIONS_DEBUG_ENABLED",
            "when enabled transitions should have better debug info")]
    public static class TransitionExt
    {
        public static Transition WithName(this Transition t, string name)
        {
            if (t != null)
            {
        ...
        public static T WithName<T>(this T t, string name) where T : class, Transition
        {
            if (t != default(T))
            {
                t.transitionName = name;
